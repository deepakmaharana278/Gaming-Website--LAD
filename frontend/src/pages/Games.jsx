...
export default function Games() {
  ...
  return (
    <Layout>
      <div className="p-6">
        <h1 className="text-3xl font-bold text-white text-center mb-6">🎮 All Games</h1>
          ...
          {filteredGames.map((game, index) => (
            <Link to={`/game/${games.indexOf(game)}`} key={index}>
              <div className="relative bg-gray-900 rounded-xl overflow-hidden shadow-lg hover:scale-105 transition duration-300 group">
                <img src={game.thumb} alt={game.title} loading="lazy" className="w-full h-40 object-cover" />
                ...
                <div className="p-3">
                  ...
                  <span className="inline-block mt-1 text-xs bg-indigo-600 text-white px-2 py-1 rounded">{game.category}</span>
                </div>
              </div>
            </Link>
          ))}
